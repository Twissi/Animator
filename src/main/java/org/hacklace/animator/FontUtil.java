...
public class FontUtil {
	...
	private final static int[][] HACKLACE_CHARSET = {
			...
			{ 0x6C, 0x1A, 0x6F, 0x1A, 0x6C }, // 8B = 139 = ^L = invader1
			{ 0x7D, 0x5A, 0x1E, 0x5A, 0x7D }, // 8C = 140 = ^M = invader2
			{ 0x4E, 0x7B, 0x0F, 0x7B, 0x4E }, // 8D = 141 = ^N = invader3
			{ 0x7C, 0x3A, 0x7E, 0x3A, 0x7C }, // 8E = 142 = ^O = invader4
			{ 0x1C, 0x76, 0x2E, 0x76, 0x1C }, // 8F = 143 = ^P = skull
			{ 0x1E, 0x34, 0x7C, 0x34, 0x1E }, // 90 = 144 = ^Q = fox
			...
			{ 0x10, 0x28, 0x24, 0x28, 0x10 }, // 9B = 155 = ^\ = UFO
			{ 0x08, 0x14, 0x2A, 0x14, 0x08 }, // 9C = 156 = ^] = diamond
			{ 0x00, 0x00, 0x00, 0x00, 0x00 }, // 9D = 157 = (not ^^) = space(5)
			{ 0x36, 0x36, 0x08, 0x36, 0x36 }, // 9E = 158 = ^_ = flower
			{ 0x1E, 0x14, 0x3C, 0x28, 0x78 }, // 9F = 159 = ^` = flag
			...
			{ 0x2A, 0x7F, 0x41, 0x7F, 0x2A }, // AA = 170 = ^k = integrated
												// circuit
			{ 0x0A, 0x00, 0x55, 0x00, 0x0A }, // AB = 171 = ^l = tree
	...
	public static int[] getIntsForRawString(String rawString,
		...
		loopOverRawString: for (int i = 0; i < rawString.length(); i++) {
			...
			assert (animationBytesForOneCharacter != null);
