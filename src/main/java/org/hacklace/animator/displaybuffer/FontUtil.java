...
public class FontUtil {
	...
	private final static int[][] HACKLACE_CHARSET = {
			...
			{0x46, 0x24, 0x1D, 0x24, 0x4C}, // A3 = 163 = ^d = person4 right
											// hand up
			{0x08, 0x44, 0x3D, 0x44, 0x08}, // A4 = 164 = ^e = person5 arms down
			{0x4C, 0x24, 0x1D, 0x24, 0x46}, // A5 = 165 = ^f = person6 left hand
											// up
	...
	public static int[] getFiveBytesForIndex(int index) {
		if (index - LOWEST_INDEX < 0
				|| index - LOWEST_INDEX >= HACKLACE_CHARSET.length) {
			return HACKLACE_CHARSET['?' - LOWEST_INDEX]; // display a question
															// mark
		}
	...
	public static int[] getMinimumBytesForIndex(int index) {
		return removeTrailingEmptyColumnsAndAddOneEmptyColumn(getFiveBytesForIndex(index));
	}
